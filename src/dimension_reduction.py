"""
Implementation of DR methods inherent in most benchmark methods to compute C_X and C_Z
"""
...
class AffinityMatcher:
    ...
    loss_fun : str, optional
        Loss used in the DR problem, by default 'kl_loss' (can be also 'square_loss')
    ...
    def __init__(
        ...
        assert optimizer in ["Adam", "SGD", "NAdam", "RAdam"]
        ...
        if init not in ["normal", "random", "pca", "WrappedNormal"] and not isinstance(
            init, torch.Tensor
        ):
    ...
    def fit_transform(self, X, y=None):
        ...
        if isinstance(self.init, torch.Tensor):  # Z embedding
            ...
            embedding = torch.normal(
                0, 1, size=(n, self.output_dim), dtype=X.dtype, device=device
            )
        elif self.init == "pca":
            ...
            embedding = geoopt.ManifoldTensor(
                sampleLorentzNormal(X.shape[0], self.output_dim + 1).double(),
                manifold=geoopt.Lorentz(),
            ).to(dtype=X.dtype, device=X.device)
        else:
            raise WrongParameter(
                'init must be in ["normal", "random", "pca", "WrappedNormal"]'
            )
        ...
        for k in pbar:
            ...
            if self.loss_fun == "kl_loss" or self.loss_fun == "kl_nomarg_loss":
                ...
                Q = self.affinity_embedding.compute_affinity(embedding)
                loss = torch.pow(PX_ - Q, 2).sum()
            else:
                raise WrongParameter(
                    '"loss_fun" should be either "kl_loss" or "square_loss".'
                )
        ...
        if self.tolog:
            ...
            if isinstance(self.affinity_embedding, BaseAffinity, ):
...
class SNEkhorn(AffinityMatcher):
    ...
    def __init__(
        ...
        sinkhorn_affinity = BistochasticAffinity(
            eps=eps,
            f=init_sinkhorn,
        ...
        super(SNEkhorn, self).__init__(
            affinity_embedding=sinkhorn_affinity,
...
class DRWrapper(AffinityMatcher):
    ...
    def __init__(
        ...
        possible_data_affinities = [
            "precomputed",
            "gaussian",
            "gaussian_bistochastic",
            "entropic_affinity",
            "symmetric_entropic_affinity",
        ]
        possible_embedding_affinities = [
            "gaussian",
            "student",
            "gaussian_bistochastic",
            "student_bistochastic",
        ]
        ...
        if affinity_embedding not in possible_embedding_affinities:
            raise WrongParameter(
                "affinity_embedding must be in {}".format(possible_embedding_affinities)
            )

        if self.perp is None and affinity_data in [
            "entropic_affinity",
            "symmetric_entropic_affinity",
        ]:
            raise WrongParameter(
        ...
        if affinity_data == "precomputed":
            ...
            if (
                "student" in params_affinity_data.keys()
                and params_affinity_data["student"] == True
            ):
                raise WrongParameter(
        ...
        if affinity_embedding == "gaussian":
            ...
            if (
                "student" in params_affinity_embedding.keys()
                and params_affinity_embedding["student"] == True
            ):
                raise WrongParameter(
            ...
            if (
                "student" in params_affinity_embedding.keys()
                and params_affinity_embedding["student"] == False
            ):
                raise WrongParameter(
        ...
        super(DRWrapper, self).__init__(
            affinity_embedding=affinity_in_Z,
